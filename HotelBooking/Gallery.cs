    ...
    public partial class Gallery : Form
    {
        ...
        private void buttonNextPic_Click(object sender, EventArgs e)
        {
            if (count < 17)
            {
                if(count>=-1 && count<=3)
                {
                    labelPicType.Text = "Room Type " + (count+2).ToString();
                }
                if (count >= 4 && count <= 8)
                {
                    labelPicType.Text = "Hall Type " + (count-3).ToString();
                }
                if(count>=9 && count<=11){
                    labelPicType.Text = "Restaurant Type " + (count - 8).ToString();
                }
                if (count >= 12 && count <= 17)
                {
                    labelPicType.Text = "Event Type " + (count - 11).ToString();
                    if (count == 12)
                    {
                        labelPicType.Text += " Conference";
                    }
                    else if(count == 13)
                    {
                        labelPicType.Text += " Party";
                    }
                    else if(count ==14){
                        labelPicType.Text += " Exhibition";
                    }
                    else if (count == 15)
                    {
                        labelPicType.Text += " Shopping Event";
                    }
                    else if(count == 16)
                    {
                        labelPicType.Text += " Wedding";
                    }
                }
                count++;
            }
            pictureBoxImgList.Image = imageList1.Images[count];
        }
